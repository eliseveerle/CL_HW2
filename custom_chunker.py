...
class _ConsecutiveNPChunkTagger(nltk.TaggerI):
    ...
    def __init__(self, feature_map, train_sents, algorithm="NaiveBayes", verbose=0, ):
        ...
        # stores the feature_map parameter as self.feature_map
        self.feature_map = feature_map
        # calls self.create_training_data on train_sents
        self.train_set = self.create_training_data(train_sents)
        # checks that algorithm is one of "NaiveBayes", "DecisionTree", "IIS", and "GIS"
        accepted = ["NaiveBayes", "DecisionTree", "IIS", "GIS"]
        if algorithm not in accepted:
            #raises an error if it's not
            raise ValueError('Your algorithm is not one of NaiveBayes, DecisionTree, IIS or GIS')
    ...
    def create_training_data(self, training_sentences):
        ...
        reformed = self.reformat_corpus_for_tagger(training_sentences)
        
        self._train_set = []
        #Turns senteces into appropriate training data
        for zin in reformed:
            hist = []
            untagged = [elem[0] for elem in zin]
        
            for i in range(len(zin)):
                #finds features of senteces
                features = self.feature_map(untagged, i, tuple(copy(hist)))
                #stores features in self._train_sent
                self._train_set.append((features, zin[i][1]))
                hist.append(zin[i][1])
